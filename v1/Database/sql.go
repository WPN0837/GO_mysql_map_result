...
func (rs *Rows) Scan(dest ...interface{}) (*map[string]interface{}, error) {
	rs.closemu.RLock()
	results := map[string]interface{}{}
	if rs.lasterr != nil && rs.lasterr != io.EOF {
		rs.closemu.RUnlock()
		return &results, rs.lasterr
	}
	if rs.closed {
	...
	if rs.lastcols == nil {
		return &results, errors.New("sql: Scan called without calling Next")
	}
	if len(dest) != len(rs.lastcols) {
		return &results, fmt.Errorf("sql: expected %d destination arguments in Scan, not %d", len(rs.lastcols), len(dest))
	}

	for i, sv := range rs.lastcols {
		result := ""
		err := convertAssignRows(&result, sv, rs)
		if err != nil {
			return &results, fmt.Errorf(`sql: Scan error on column index %d, name %q: %v`, i, rs.rowsi.Columns()[i], err)
		}
		switch d := dest[i].(type) {
		case *string:
			results[*d] = result
		case *interface{}:
			results[(*d).(string)] = result
		}
	}
	return &results, nil
}
...
func (r *Row) Scan(dest ...interface{}) (*map[string]interface{}, error) {
	...
	for _, dp := range dest {
		if _, ok := dp.(*RawBytes); ok {
			return result, errors.New("sql: RawBytes isn't allowed on Row.Scan")
		}
	}
